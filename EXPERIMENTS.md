...
| 21 | 09-01 | **Axis ablation — is provenance distinct from taxonomy?** (decisive control) | Compare curated vs iNat-only strata *within a single pollinator order*, so taxon composition is held fixed. | **Mostly NOT distinct — deflates the leakage headline.** Uncontrolled curated/iNat ratio for gbm_geo is 2.3× (0.471 vs 0.208); **within Hymenoptera only it falls to 1.33×** (0.534 vs 0.402), and the process-blind N baseline itself shows 1.13× — so part of the residual is "curated pairs are simply more canonical," not model leakage. Consistent with the probe control (family alone predicts source at 0.824 of BioCLIP's 0.914). Also: models are far better on bees (0.53) than butterflies (0.30). **Conclusion: report provenance as a confounded, composition-driven axis, not as a clean shortcut-detection instrument.** | `axis_ablation_v1.csv` |
