...
**Correction (added after Step 7):** the `plant_pollinator_edges.csv` file used here was later found to contain 15 self-paired rows out of 54 (the identical species name listed as both `plant_species` and `pollinator_species`, e.g. *Bellis perennis* – *Bellis perennis`). These are not valid interactions. The effective edge count going into this analysis was therefore **39, not 54**. The "12 edges survived" figure above is consequently 12 of 39 valid candidate edges (31%), not 12 of 54 (22%) as originally reported. This does not change which 12 edges survived or any of the computed overlap values, only the correct denominator for interpreting the survival rate. The edge list should be corrected at the source before any future rerun (see Step 8).

The original framing otherwise still holds: of the 39 valid edges, 27 were dropped because no spatial bin had both species recorded with ≥10 observations, reflecting sparse co-located observation coverage in the underlying dataset rather than a true absence of interaction. The 12 surviving edges are dominated by *Asclepias syriaca* (7 of 12), a well-known pollinator generalist with dense observational coverage.
