...
**Concurrent work (read 2026-09-08; PDF in docs/reference/).** Baiotto, Cosma, ..., Li & Guzman, *Local interaction networks reconstructed from global biodiversity data improve pollinator restoration decision making* (NECTAR), bioRxiv 2026-04-01, https://www.biorxiv.org/content/10.64898/2026.03.30.715389v1. Cornell / CBI / CNPS / UCSB (Seltmann) et al.
- **Inference rule (Eq. 1):** for pairs passing a taxonomic constraint (pollinator recorded with the plant's *genus*; t-SVD graph embedding + phylogenetic ancestral-state transfer, after Strydom et al., for pollinators with no records), plausibility in ecoregion k is `p_ijk = p_co-occ * p_co-phen = [sum_s p_occ(i,s) p_occ(j,s) / area_k] * [|D_i ∩ D_j| / |D_j|]` -- a **product of a spatial marginal and a phenological marginal**, then thresholded at the 25th percentile of confirmed pairs. This is precisely the marginalised construction our ladder shows loses most of the per-cell signal; it is the natural comparative model for us and is re-implemented as `baselines/nectar_like.py` (gated and ungated).
- **Inputs:** ensemble SDMs at 5 arcmin (50 predictors, target-group background, tiered by sample size, OR10 binary maps); phenometrics from circular von Mises fits (5th-95th percentiles), tiered; 287,231 interaction records (GloBI, HOSTS, CropPol, Discover Life scrape, Pollen Specialist Bees, Doneski butterflies) -> 26,007 unique species-level CA-native pairs; 5,745 with a georeferenced CA locality.
- **Evaluation:** withhold 20% of the 5,745 spatially explicit pairs, recall of withheld pairs after thresholding: **53.8%**, vs 0.1% random among co-occurring species and 25.2% for the phylogenetically constrained null. Recall only -- no negatives, no AUPR/AUROC, no cold-species split. Their stated limitation: they downscale metawebs to ecoregions, not to realised local networks.
- **Data:** Zenodo 10.5281/zenodo.19338963 (457.7 MB) is **embargoed until 2026-10-01**; the GitHub repo (EntoDataSciCornell/NECTAR) is code only. After the embargo, their species-level records from non-GloBI sources (Discover Life, CA Lepidoptera resource, Pollen Specialist Bees, Doneski) would be a genuinely external positive-only test set for California; the predicted metaweb itself is model output, not ground truth.
