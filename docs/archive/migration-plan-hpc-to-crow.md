...
## RECOVERY NOTE (crow, 2026-09-04)

`pipelines-port.bundle` could not be fetched: it is a thin bundle whose prerequisite commit
`d738f01` ("Update eda-questions.md") does not exist in the crow repository, and one delta base
blob (`d74fd943…`, the edited `docs/paper/eda-questions.md`) was therefore unresolvable.

Recovered by unpacking the bundle's packfile directly (57 of 65 objects readable) and walking the
surviving trees: **all 16 files of `pipelines/` and `MIGRATION_PLAN.md` were restored intact**.
Lost: Dan's commit history for these files, and `configs/pipelines.yaml`, which lived in the broken
root tree. The yaml was reconstructed from the config keys the code actually reads and repointed at
crow paths.

Consequence: this branch carries the same *content* as `pipelines-port` but different commit SHAs.
If the original history matters, push `d738f01` from the HPC and re-fetch.
