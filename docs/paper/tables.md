...
Paper-ready skeletons. `—` marks a cell to populate. Metrics and evaluation sets are fixed in
[evaluation-protocol.md](evaluation-protocol.md): primary is per-plant **normalised recall@10**, with
recall@{10,20}, nDCG@{10,20}, median rank of the first true partner, and **PR-AUC at the network's
connectance**. Scoring is restricted to Tier A; training uses both tiers.

Generated by `python -m antheia.eval.tables` from the bundles — do not edit populated cells by hand.
