...
def main():
    ...
    rec = {n: {"plant": [], "r10": [], "r50": [], "hit": [], "r10_t1": []} for n in names}
    for i, (sp, part) in enumerate(partners_all.items()):
        ...
        for n, s in svec.items():
            ...
            rec[n]["r10"].append(len(part & t10) / len(part))
            ...
            if pt1:
                ...
                rec[n]["r10_t1"].append(len(pt1 & t10m) / len(pt1))
        if (i + 1) % 150 == 0:
            print(f"  eval {i + 1}/{len(partners_all)}", flush=True)

    rows = []
    for n in names:
        pd.DataFrame({"plant": rec[n]["plant"], "r10": rec[n]["r10"], "r50": rec[n]["r50"],
                      "hit": rec[n]["hit"]}).to_csv(out_dir / f"{n}.csv", index=False)
        r10, lo, hi, _ = bootstrap_mean(np.array(rec[n]["r10"]), nboot, seed)
        t10, tlo, thi, _ = bootstrap_mean(np.array(rec[n]["r10_t1"]), nboot, seed)
        rows.append({"model": n, "r10_all": r10, "lo": lo, "hi": hi,
                     "r50_all": np.mean(rec[n]["r50"]), "hit10_all": np.mean(rec[n]["hit"]),
                     "r10_t1": t10, "t1_lo": tlo, "t1_hi": thi})
        print(f"  {n:<20} R@10 {r10:.4f} [{lo:.4f},{hi:.4f}]  R@50 {np.mean(rec[n]['r50']):.4f}"
              f"  hit {np.mean(rec[n]['hit']):.4f}  T1 {t10:.4f}", flush=True)
    table = pd.DataFrame(rows)
    table.to_csv(cfg["edges"].parent / "final_table_v1.csv", index=False)

    # Paired plant-level bootstrap on r10_all.
    rng = np.random.default_rng(seed)
    R = {n: np.array(rec[n]["r10"]) for n in names}
    ...
    pd.DataFrame(sig).to_csv(cfg["edges"].parent / "final_significance_v1.csv", index=False)
