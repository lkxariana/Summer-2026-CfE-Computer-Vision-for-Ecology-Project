...
# Plant-side temporal REPRESENTATION: marginalized 52-week curve vs spatiotemporal trajectory.
# Pollinator-side curve is held constant across variants, so this isolates the plant encoding.
GRID = [
    ("none", "none", "none"),
    ("curve", "curve", "none"),
    ("trajectory", "traj", "none"),
    ("curve+trajectory", "both", "none"),
]


def main():
    ap = argparse.ArgumentParser(description="Plant-side temporal representation: curve vs spatiotemporal trajectory")
    ...
    ap.add_argument("--tag", default="trajectory_v1")
    args = ap.parse_args()
    ...
    p_rest = p_full[:, 52:].copy()                       # [log range | embedding]
    P_VARIANTS = {
        "none": p_rest,
        "curve": p_full,
        "traj": np.hstack([traj, p_rest]),
        "both": np.hstack([p_full[:, :52], traj, p_rest]),
    }
    print(f"data ready ({time.time() - t0:.0f}s); plant tower dims " +
          ", ".join(f"{k}={v.shape[1]}" for k, v in P_VARIANTS.items()), flush=True)
    ...
    for name, pvar, dmode in GRID:
        data.p_dense = P_VARIANTS[pvar]
        data.q_dense = q_full
        tv = time.time()
    ...
    for h in ("retrieval", "compatibility"):
        for a, b in (("curve", "none"), ("trajectory", "none"),
                     ("trajectory", "curve"), ("curve+trajectory", "curve")):
