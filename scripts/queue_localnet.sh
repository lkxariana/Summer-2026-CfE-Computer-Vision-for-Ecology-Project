...
run () {
  ...
  if [ "$DEV" = "cpu" ]; then
    CUDA_VISIBLE_DEVICES="" $P eval/run_localnets.py --model $MODEL --name $NAME --config "$CFG" --seed $SEED --device cpu 2>&1 | grep -v Warning | grep -E "networks|Traceback|Error"
  else
    CUDA_VISIBLE_DEVICES=$DEV $P eval/run_localnets.py --model $MODEL --name $NAME --config "$CFG" --seed $SEED 2>&1 | grep -v Warning | grep -E "networks|Traceback|Error"
  fi
}
if [ "$DEV" = "cpu" ]; then
  for M in popularity cooccurrence abundance phenology_abundance congeneric svd_taxonomic antheia_spatial antheia_scalar nectar_ungated nectar_like pair_gbm ours_gbm routed; do
    run baseline_$M $M '{}' 42
  done
else
  for SEED in "$@"; do
