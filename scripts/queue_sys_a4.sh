#!/usr/bin/env bash
# System on the A4 retriever (no taxon nodes + pair co-presence term); within-site scored with the affinity-only retriever + identity re-ranker: cold_plant s42, local networks, cold_poll, cold_both, warm s42. GPU = $1.
set -u
