...
for SEED in 0 1; do
  ls runs/M4.0_system_sym/*/localnet/s$SEED/metrics.json >/dev/null 2>&1 && { echo "[skip] localnet s$SEED"; continue; }
  echo "[run] M4.0_system_sym localnet s$SEED $(date +%H:%M)"
  CUDA_VISIBLE_DEVICES=$GPU $P eval/run_localnets_fusion.py --name M4.0_system_sym --retriever-model rgcn --retriever-config "$RCFG" --config "$FCFG" --seed $SEED 2>&1 | grep --line-buffered -v Warning | grep --line-buffered -E "networks|Traceback|Error"
done
