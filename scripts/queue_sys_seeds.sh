...
for SEED in 0 1; do for SPLIT in cold_both warm; do
  ls runs/M3.7_rgcn_sym/*/$SPLIT/val/s$SEED/metrics.json >/dev/null 2>&1 && { echo "[skip] M3.7 $SPLIT s$SEED"; continue; }
  echo "[run] M3.7_rgcn_sym $SPLIT s$SEED $(date +%H:%M)"
  CUDA_VISIBLE_DEVICES=$GPU $P eval/run_ladder.py --model rgcn --name M3.7_rgcn_sym --config "$CFG" --split $SPLIT --seed $SEED 2>&1 | grep --line-buffered -v Warning | grep --line-buffered -E "AUPR|Traceback|Error"
done; done
