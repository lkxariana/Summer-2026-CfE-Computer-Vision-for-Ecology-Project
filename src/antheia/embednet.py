...
class PairHead(nn.Module):
    ...
    def forward(self, hp, hq):
        ...
        x = torch.cat([a, b, a * b, (a - b).abs()], -1)           # [B, C, 4d]
...
class EmbedRanker:
    ...
    def fit(self, edges, store):
        ...
        self.head = PairHead(cfg.d_model, cfg.hidden, cfg.dropout).to(dev)
    ...
    def score_plant(self, p, chunk=4096):
        ...
        with torch.no_grad():
            ...
            for s in range(0, self.n_q, chunk):
                ...
                out[s:s + len(hq)] = self.head(hp, hq).squeeze(0).float().cpu().numpy()
        return out
