...
The reported system uses `identity_only`: [CLS] and the two identity tokens, no field tokens. Its two
appendix variants replace the frozen text identity with the retriever's own vectors -- `id_source`
"retriever_proj" (its projected names) or "retriever_h" (its graph outputs) -- passed in through
`fit(id_override=)`.
"""
